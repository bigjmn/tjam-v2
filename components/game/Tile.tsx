...
export function TileComponent({
	...
	useEffect(() => {
		...
		if (offsetX.value === 0 && offsetY.value === 0) {
			positionX.value = withSpring(targetX, {
				damping: 20,
				stiffness: 300,
			});
			positionY.value = withSpring(targetY, {
				damping: 20,
				stiffness: 300,
			});
		}
	...
	useEffect(() => {
		if (tile.isHomeRowExiting) {
			...
			positionX.value = withDelay(
				delay,
				withTiming(positionX.value - 380, {
					duration: SLIDE_DURATION,
	...
	const handleDrop = (currentX: number, currentY: number) => {
		...
		if (clampedRow === 1) {
			positionX.value = withSpring(getTargetX(tile.x), {
				damping: 20,
				stiffness: 300,
			});
			positionY.value = withSpring(getTargetY(tile.y), {
				damping: 20,
				stiffness: 300,
			});
		} else {
			positionX.value = withSpring(getTargetX(clampedCol), {
				damping: 20,
				stiffness: 300,
			});
			positionY.value = withSpring(getTargetY(clampedRow), {
				damping: 20,
				stiffness: 300,
			});
		}
	...
	const panGesture = Gesture.Pan()
		...
		.onUpdate((event) => {
			...
			const screenX =
				positionX.value + event.translationX + GRID_CELL_SIZE / 2;
			const screenY =
				positionY.value + event.translationY + GRID_CELL_SIZE / 2;
