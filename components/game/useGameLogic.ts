...
function dealNewTiles(): Tile[] {
	...
	return word
		.split("")
		.map((letter, index) => createTile(letter, index, HOME_ROW));
}
...
export function useGameLogic() {
	...
	const moveTile = useCallback(
		(tileId: string, newX: number, newY: number) => {
			setTiles((currentTiles) => {
				...
				if (newX < 0 || newX > 2 || newY < 0 || newY > 4 || newY === 1)
					return currentTiles;
	...
	const tileStats = useMemo(() => {
		...
		const newTilesOnBoard = tiles.filter(
			(t) => t.isNew && t.y >= BOARD_START_ROW,
		);
	...
	const canCommit = useMemo(() => {
		return (
			tileStats.newTilesOnBoard.length === 2 &&
			tileStats.newTilesInHome.length === 1
		);
	}, [tileStats]);
