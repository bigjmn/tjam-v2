...
// }
function Tile({id, letter, startx, starty, givePos, takenSpots, canMove, partValid, claimMovement, inMotion}:TileProps){
  ...
  const moveTile = useCallback(
    (to:string) => {
      ...
      const dest = takenSpots.includes(to) ? sitsOn.value : allSquares().includes(to) ? to : sitsOn.value
    ...
    [isPressed, offsetX, offsetY, translateX, translateY, sitsOn, takenSpots, partValid]
  )
  ...
  const animatedStyles = useAnimatedStyle(() => {
    return {
      ...
      backgroundColor: canMove ? 'tan' : partValid.includes(sitsOn.value) ? 'rgba(0,255,0,.2)' : '#e2e2e2'
  ...
  const underlay = useAnimatedStyle(() => {
    ...
    return{
      ...
      zIndex:0,
      backgroundColor: ['00','10','20','02','12','22','03','13','23','04','14','24'].includes(position) ? '#aaa' : "transparent"
