...
export function OverlappingTabsDeck({
	...
	return (
		<View
			style={[{ width: screenWidth, height, overflow: "visible" }, style]}
		>
			{cards.map((card, i) => (
				<DeckStackCard
					key={card.key}
				...
				<View style={styles.cardPressable}>
					<View style={styles.headerRow}>
						<ThemedText
							style={[
								styles.title,
								{ color: card.headerTextColor ?? "#111" },
							]}
							numberOfLines={1}
						>
							{card.title}
					...
					<Animated.View
						style={[styles.expandedArea, animatedInnerStyle]}
					>
						{card.renderExpanded?.() ?? (
							<ScrollView
