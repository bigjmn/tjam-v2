...
import { scoringAchievements, streakingAchievements, noveltyAchievements, secretAchievements, legendaryAchievements } from "../utils/achievements"
...
export const useAchievements = () => {
    ...
    const scoringGoal = scoringAchievements.sort((a, b) => a.scoreThreshhold - b.scoreThreshhold).find(s => s.scoreThreshhold > playerStats.topScore)
    const streakingGoal = streakingAchievements.sort((a, b) => a.streakScore - b.streakScore).find(s => !playerStats.achievementsWon.includes(s.key))
    const noveltyGoal = noveltyAchievements.filter(na => !playerStats.achievementsWon.includes(na.key))[0]

    const gameAchievements = (turns: TurnInfo[]) => {
        const allAchievements:string[] = []
        const wordGroups = turns.map(t => t.wordsMade).flat()
        const allWordsMade = turns.map(t => t.wordsMade).flat().flat()
        ...
        // ewe kid 
        if (allWordsMade.includes("EWE") && allWordsMade.includes("YEW") && allWordsMade.includes("YOU")){
