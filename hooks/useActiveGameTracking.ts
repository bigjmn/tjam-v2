...
export function useActiveGameTracking() {
	...
	const startTracking = async (data?: Partial<ActiveGameData>) => {
		...
		console.log(
			"🎮 [GameTracking] Starting active game tracking:",
			activeGame,
		);
		...
		try {
			...
			console.error(
				"🎮 [GameTracking] ❌ Failed to write active game:",
				err,
			);
		}
	};
	...
	const endTracking = async () => {
		...
		try {
			...
			console.error(
				"🎮 [GameTracking] ❌ Failed to clear active game:",
				err,
			);
		}
	};
		...
		try {
			...
			if (activeGameJson) {
				...
				if (!activeGame.variant) {
					...
					console.log(
						"🎮 [GameTracking] Skipping abandoned record - was variant:",
						activeGame.variant,
					);
				}
			...
			console.error(
				"🎮 [GameTracking] ❌ Error checking for orphaned game:",
				err,
			);
		}
	};
