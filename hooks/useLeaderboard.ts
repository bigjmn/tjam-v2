...
import { query, limit, orderBy, getDocs, doc, getDoc, collection } from 'firebase/firestore'
...
export const useLeaderboard = () => {
    ...
    const getLeaders = async () => {
        const leadersRef = collection(firestore, "users").withConverter(playerStatConverter)
        setIsLoading(true)
        setBoardErr(null)

        try {
            const playerRanks = await getDocs(leadersRef).then((ld) => {
                return ld.docs.map(d => d.data()).filter(d => d.username !== null).map(d => playerToLeader(d))
            })
            const globalList:GlobalLeader[] = playerRanks.toSorted((a, b) => b.bestAllTime - a.bestAllTime).map((pRank, i) => ({...pRank, type:"global", globalRank:i+1 }))
            const weeklyList:WeeklyLeader[] = playerRanks.toSorted((a, b) => b.bestWeek - a.bestWeek).map((pRank, i) => ({...pRank, type:"weekly", weeklyRank: i+1}))
