...
export const useStats = () => {
	...
	const getStats = () => {
		...
		const joinDate = playerStats.dateJoined
			? moment(playerStats.dateJoined).format("M/DD/YYYY")
			: "";
	...
	const wodStats = () => {
		const wodRate = playerStats.dateJoined
			? wodPct(playerStats.dateJoined, playerStats.achievementsWon)
			: -1;
		const { longestStreak, currentStreak } =
			getLongestStreakFromAchievements(playerStats.achievementsWon);

		return { wodRate, longestStreak, currentStreak } as WordOfDayStats;
	};

	const getVariantBests = async () => {
		const defaultBest: VariantBests = {
			scrabble: 0,
			fours: 0,
			fiveline: 0,
		};
