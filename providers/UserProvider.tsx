...
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, onAuthStateChanged, signOut, User, updateProfile, FacebookAuthProvider, signInWithCredential } from 'firebase/auth'
...
import { convertOldPlayerOb, createPlayer, converter, playerStatConverter } from '../utils/helpers'
interface UserContextProps {
    user : User | null; 
    signInWithGoogle: () => Promise<void>;
    login: (email:string, password:string) => Promise<void>;
    register: (email:string, password:string, confirmPassword:string, username:string, name:string) => Promise<void>;
    logout: () => Promise<void>;
    authChecked: boolean;
...
export function UserProvider({children} : { children: React.ReactNode }){
    ...
    const signInWithGoogle = async () => {
        try {
            ...
            const udocRef = doc(firestore, "users", googleUserData.uid).withConverter(playerStatConverter)
            const uDoc = await getDoc(udocRef)
            if (!uDoc.exists()){
                const todayDisplay = moment(new Date()).format("dddd, MMM DD, YYYY")
                ...
                await setDoc(udocRef, {...playerStats, email:userEmail, username:newUsername})
                await setDoc(doc(firestore, "usernames", newUsername), {userid: googleUserData.uid})
    ...
    async function register(email:string, password:string, confirmPassword:string, username:string, name:string) {
        ...
        try {
            ...
            const todayDisplay = moment(new Date()).format("dddd, MMM DD, YYYY")
            await setDoc(userDocRef, {email, username, name, todayAnswer:"", votesCast: 0, rating: 1200, joinDate: todayDisplay, gold: 0, silver: 0, bronze: 0, answerRate:0})
    ...
    return (
        <UserContext.Provider value={{user, signInWithGoogle, login, register, logout, authChecked, playerStats}}>
            { children }
