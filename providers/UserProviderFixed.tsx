...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const signInWithApple = async () => {
    ...
    try {
      if (playerStats === null) {
        console.error("🍎 [Apple] ERROR: playerStats is null - cannot proceed");
      ...
      if (!uDoc.exists()) {
        ...
        if (uNameDoc.exists() && uNameDoc.data().userid !== uid) {
          ...
          console.log(`🍎 [Apple] Username collision: "${oldUsername}" → "${newUsername}"`);
        }
        ...
        if (finalUsername !== preferredUsername) {
          console.log(`🍎 [Apple] Username collision: "${preferredUsername}" → "${finalUsername}"`);
        }
        ...
        console.log("🍎 [Apple] ✓ Existing user stats merged successfully");
      }
    } catch (e) {
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      if (playerStats === null) {
        console.error("🔵 [Google] ERROR: playerStats is null - cannot proceed");
        throw new Error("Stats not loaded yet. Please try again.");
      }
      ...
      if (!googleCred) {
        console.error("🔵 [Google] ERROR: Failed to get Google credential");
        throw new Error("Google sign-in failed. Please try again.");
      }

      console.log("🔵 [Google] Credential obtained, attempting link or sign-in...");
      ...
      if (!uDoc.exists()) {
        ...
        if (uNameDoc.exists() && uNameDoc.data().userid !== uid) {
          const oldUsername = newUsername;
          newUsername += usernameNumberTail();
          console.log(`🔵 [Google] Username collision: "${oldUsername}" → "${newUsername}"`);
        }
        ...
        if (finalUsername !== preferredUsername) {
          console.log(`🔵 [Google] Username collision: "${preferredUsername}" → "${finalUsername}"`);
        }
        ...
        console.log("🔵 [Google] ✓ Existing user stats merged successfully");
      }
    } catch (err) {
  ...
  async function logout() {
    ...
    try {
      ...
      if (playerStats) {
        ...
        console.log("🚪 [Logout] Stripping auth fields from local stats:", {
        ...
        console.log("🚪 [Logout] ✓ Stripped stats saved to AsyncStorage");
      }

      // FIX: Don't call signInAnonymously here - let the auth listener handle it
      console.log("🚪 [Logout] Waiting for auth listener to sign in anonymously...");
    } catch (err) {
  ...
  async function getUserInfo() {
    ...
    try {
      ...
      if (oldPlayerJson !== null) {
        console.log("💾 [Hydration] Found v1 stats - migrating to v2...");
      ...
      console.log("💾 [Hydration] No existing stats - creating fresh player");
  ...
  useEffect(() => {
    ...
    if (user) {
      console.log(`👤 [Auth] Current user: ${user.uid} (anonymous: ${user.isAnonymous})`);
    }
  }, [playerStats, user]);
  ...
  useEffect(() => {
    ...
    if (user.isAnonymous && (playerStats.username || playerStats.email)) {
      ...
      AsyncStorage.setItem(PLAYER_V2_KEY, JSON.stringify(remainingStats))
        .then(() => console.log("🔒 [Invariant] ✓ Stripped stats saved"))
        .catch((err) => console.error("🔒 [Invariant] ❌ Failed to save stripped stats:", err));
    }
  ...
  const makeOrGetDoc = async () => {
    ...
    if (!uDoc.exists()) {
      console.log("📄 [Firestore] Doc doesn't exist - creating safety doc");
      console.log("📄 [Firestore] This should only happen if sign-in flow failed");
  ...
  const anonSignIn = async () => {
    ...
    if (!anonSignInRef.current) {
      ...
      anonSignInRef.current = signInAnonymously(auth)
        .then((uanon) => {
          console.log("👻 [Anon] ✓ Anonymous sign-in successful:", uanon.user.uid);
          return uanon.user;
        })
        .catch((e) => {
      ...
      console.log("👻 [Anon] Anonymous sign-in already in progress, waiting...");
    }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        if (statsHydrated) {
          console.log("🔄 [Auth] ✓ Auth checked (stats already hydrated)");
          setAuthChecked(true);
        } else {
          console.log("🔄 [Auth] ⏳ Waiting for stats to hydrate before marking ready...");
        }
      } else {
