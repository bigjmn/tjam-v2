...
export const playerStatConverter = {
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot): PlayerStats {
    ...
    return {
      ...
      gameHist: (data.gameHist || []).map((record: any) => ({
        ...record,
...
export const mergeStats = (
  ...
  const mergedAchs = Array.from(
    new Set([...localStats.achievementsWon, ...remoteStats.achievementsWon]),
  );
  ...
  const uniqueRecords = combinedRecords.filter(
    (record, index, self) =>
      ...
      self.findIndex(
        (t) =>
          toDate(t.timestamp).getTime() === toDate(record.timestamp).getTime(),
      ),
  );
  const sortedRecords = uniqueRecords
    .map((record) => ({ ...record, timestamp: toDate(record.timestamp) }))
  ...
  const dj = localDate < remoteDate ? localDate : remoteDate;
  return {
    achievementsWon: mergedAchs,
    topScore: mergedBest,
    numGames: numGames,
    id: id,
    gameHist: sortedRecords,
    dateJoined: dj,
    username: uName,
    email: uEmail,
  } as PlayerStats;
...
export function calculateStreaks(dates: Date[]): StreakResult {
  ...
  if (lastUniqueDay) {
    if (
      lastUniqueDay.isSame(today, "day") ||
      lastUniqueDay.isSame(yesterday, "day")
    ) {
      currentStreak = run;
    }
  }
