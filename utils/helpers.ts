...
import uuid from "react-native-uuid"
  import { PartialWithFieldValue, QueryDocumentSnapshot, DocumentData } from "firebase/firestore";          
...
export const convertOldPlayerOb = (oldPlayerOb:any):PlayerStats => {
  ...
  const prevAch:string[] = Object.keys(oldPlayerOb).filter(k => oldPlayerOb[k] == true)
  const achSet = new Set([...scoreAchievementsWon, ...prevAch])
  const achievementsWon = Array.from(achSet)
  const numGames:number = !oldPlayerOb.scoreHist ? 0 : oldPlayerOb.scoreHist.length
...
export const playerStatConverter = {
    toFirestore(playerStats:PlayerStats) : DocumentData {
        return { id: playerStats.id, topScore: playerStats.topScore, numGames: playerStats.numGames, gameHist: playerStats.gameHist, achievementsWon: playerStats.achievementsWon, email: playerStats.email, username: playerStats.username}
    },
    fromFirestore(snapshot: QueryDocumentSnapshot) : PlayerStats {
        const data = snapshot.data()! 
        return {id:data.id, topScore:data.topScore, numGames:data.numGames, gameHist: data.gameHist, achievementsWon: data.achievementsWon, username: data.username, email: data.email, name: data.name} as PlayerStats
    }
}
